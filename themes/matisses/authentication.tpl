...
{else}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box account-creation_form grid_12">
		...
		<div class="account_creation grid_12 omega alpha">
			...
			<div class="cf">
				...
				<div class="required password form-group grid_6">
					<label for="passwd" class="grid_5 alpha omega">{l s='Password'} <sup>*</sup></label>
					<!-- <span class="form_info grid_10 alpha omega">{l s='(Five characters minimum)'}</span> -->
		...
		{/if}
		{$HOOK_CREATE_ACCOUNT_FORM}
		<div class="submit cf grid_12">
			<input type="hidden" name="email_create" value="1" />
			<input type="hidden" name="is_new_customer" value="1" />
			{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
			<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button btn-red">
				{l s='Crear cuenta'}
			</button>
		</div>
	</form>

...
<div class="twa container both-style {if $displayType == 1} {else} no-carousel-reviews{/if}">
	<h2 class="title_main_section"><span>{l s='Testimonials' mod='testimonialswithavatars'}</span></h2>
	{if $displayType == 2}
	<div class="inner-twa-reviews">
	...
	{/if}
	<button id="addNew" class="btn btn-default">
	...
	<div class="addForm row clearfix" style="display:none;">		
		<form id="add_new_post" enctype="multipart/form-data">
			...
			<div class="new_content_wrapper">
					...
					<div class="field editable_rating">
						<div class="stars_holder">
							...
							{for $r=1 to $general_settings.rating_num}
								<i class="rating_star fa fa-{$general_settings.rating_class|escape:'html'}" data-rating="{$r|intval}"></i>
							{/for}						
						</div>
						<br />
				...
				<button class="btn btn-default" id="submitPost">{l s='OK' mod='testimonialswithavatars'}</button>
			</div>
		</form>
	</div>
</div>
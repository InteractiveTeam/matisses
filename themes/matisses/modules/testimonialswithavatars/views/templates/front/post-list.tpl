...
<div class="post" data-idpost="{$post.id_post|intval}">	
	<div class="post_avatar text-center">
		<img src="{$twa->getAvatarPath($post.avatar|escape:'html')}" alt="{$post.customer_name|escape:'html'}">
		{if $general_settings.rating_num}
			<span class="post_rating">
				{for $r=1 to $general_settings.rating_num}
					<i class="rating_star fa fa-{$general_settings.rating_class|escape:'html'}{if $post.rating >= $r} on{/if}"></i>
				{/for}
			</span>
		{/if}
	</div>

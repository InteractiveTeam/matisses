										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="javascript:void(0)" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>
					...
					{if isset($filter.values)}
                        ...
                        <div class="layered_subtitle_heading">
                            ...
                            <!--<span class="layered_close">
                            	<a href="javascript:void(0)" data-rel="ul_layered_{$filter.type}_{$filter.id_key}"></a>
                            </span>-->
						</div>

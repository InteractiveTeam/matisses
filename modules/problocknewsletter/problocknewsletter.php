...
class Problocknewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('PRO Newsletter block');
	...
	{
		if (!parent::install() || !$this->registerHook(array('header', 'footerTop', 'actionCustomerAccountAdd')))
			return false;
	...
	{
		if (Tools::getValue('email') == false || !Validate::isEmail(Tools::getValue('email')))
	...
	protected function sendVerificationEmail($email, $token)
	{
		$verif_url = Context::getContext()->link->getModuleLink(
			'problocknewsletter', 'verification', array(
				'token' => $token,
			)
		);
			...
			if ($this->error)
			{
				$this->smarty->assign(
					array(
						'color' => 'red',
						'msg' => $this->error,
						'nw_value' =>  Tools::getIsset(Tools::getValue('email')) ? pSQL(Tools::getValue('email')) : false,
						'nw_error' => true,
	...
	public function hookDisplayLeftColumn($params)
	{
		...
		return $this->display(__FILE__, 'problocknewsletter.tpl');
	}
	...
	{
		$this->context->controller->addCSS(($this->_path).'views/css/problocknewsletter.css', 'all');
		$this->context->controller->addJS($this->_path.'views/js/problocknewsletter.js', 'all');
	}
	...
	{
		//if e-mail of the created user address has already been added to the newsletter through the problocknewsletter module,
		//we delete it from problocknewsletter table to prevent duplicates

...
<div class="alert alert-warning" id="{$list_id}-empty-filters-alert" style="display:none;">{l s='Please fill at least one field to perform a search in this list.' mod='prestacenterxmlexportfree'}</div>
		...
		<div class="panel-heading">
			...
			{foreach from=$toolbar_btn item=btn key=k}
				{if $k != 'modules-list' && $k != 'back'}
					<a id="desc-{$table}-{if isset($btn.imgclass)}{$btn.imgclass}{else}{$k}{/if}" class="list-toolbar-btn" {if isset($btn.href)}href="{$btn.href|escape:'html':'UTF-8'}"{/if} {if isset($btn.target) && $btn.target}target="_blank"{/if}{if isset($btn.js) && $btn.js}onclick="{$btn.js}"{/if}>
						<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s=$btn.desc mod='prestacenterxmlexportfree'}" data-html="true">
			...
			{/foreach}
				<a id="desc-{$table|strval}-refresh" class="list-toolbar-btn" href="javascript:location.reload();">
					<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Refresh list' mod='prestacenterxmlexportfree'}" data-html="true">
				...
				$(function() {
					...
					if (btn_save.length > 0 && btn_submit.length > 0)
					{
						...
						if (btn_save_and_stay.length > 0)
						{
							...
							{
								lbl_save_and_stay.html(btn_submit.val() + " {l s='and stay' mod='prestacenterxmlexportfree'} ");
							}
			...
			<thead>
				<tr class="nodrag nodrop">
					...
					<th>
						...
						{else}
							{l s='Group shop' mod='prestacenterxmlexportfree'}
						{/if}
						</span>
					</th>
					{/if}
							...
							{elseif $params.type == 'date' || $params.type == 'datetime'}
								<div class="date_range row">
									<div class="input-group fixed-width-md row-margin-bottom">
										<input type="text" class="filter datepicker date-input form-control" id="{$params.id_date}_0" name="{$params.name_date}[0]" value="{if isset($params.value.0)}{$params.value.0}{/if}" placeholder="{l s='From' mod='prestacenterxmlexportfree'}"/>
										<span class="input-group-addon">
											<i class="icon-calendar"></i>
										</span>
									</div>
									<div class="input-group fixed-width-md">
										<input type="text" class="filter datepicker date-input form-control" id="{$params.id_date}_1" name="{$params.name_date}[1]" value="{if isset($params.value.1)}{$params.value.1}{/if}" placeholder="{l s='To' mod='prestacenterxmlexportfree'}" />
										<span class="input-group-addon">
											<i class="icon-calendar"></i>
										</span>
									</div>
					...
					<th class="actions">
						...
						<span class="pull-right">
							...
							<button type="submit" id="submitFilterButton{$list_id|strval}" name="submitFilter" class="btn btn-default" data-list-id="{$list_id|strval}">
								<i class="icon-search"></i> {l s='Search' mod='prestacenterxmlexportfree'}
							</button>
							{if $filters_has_value}
							<button type="submit" name="submitReset{$list_id|strval}" class="btn btn-warning">
								<i class="icon-eraser"></i> {l s='Reset' mod='prestacenterxmlexportfree'}
							</button>
							{/if}
						</span>
						{/if}

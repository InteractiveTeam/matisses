...
<form id="{if isset($fields.form.form.id_form)}{$fields.form.form.id_form|escape:'html':'UTF-8'}{else}{if $table == null}configuration_form{else}{$table}_form{/if}{/if}" class="defaultForm {$name_controller} form-horizontal" action="{$current}&amp;token={$token}" method="post" enctype="multipart/form-data" {if isset($style)}style="{$style}"{/if} novalidate>
	{if $form_id}
		<input type="hidden" name="{$identifier|strval}" id="{$identifier|strval}" value="{$form_id|strval}" />
	{/if}
	{if !empty($submit_action)}
		<input type="hidden" name="{$submit_action|strval}" value="1" />
	{/if}

	{foreach $fields as $f => $fieldset}
		{block name="fieldset"}
		<div class="panel" id="fieldset_{$f|strval}">
			{foreach $fieldset.form as $key => $field}
				...
				{elseif $key == 'description' && $field}
					<div class="alert alert-info">{$field|strval}</div>
				{elseif $key == 'input'}
						...
						{else}
							...
							{block name="field"}
									...
									{foreach $languages as $language}
										...
										{/if}
												...
												<span id="{if isset($input.id)}{$input.id}_{$language.id_lang}{else}{$input.name}_{$language.id_lang}{/if}_counter" class="input-group-addon">
													<span class="text-count-down">{$input.maxchar|strval}</span>
												</span>
												{/if}
												...
												<input type="text"
													...
													name="{$input.name|strval}_{$language.id_lang|strval}"
										...
										{if $languages|count > 1}
											...
											<div class="col-lg-2">
												<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
													{$language.iso_code|strval}
													<i class="icon-caret-down"></i>
												</button>
												<ul class="dropdown-menu">
													{foreach from=$languages item=language}
													<li><a href="javascript:hideOtherLanguage({$language.id_lang|strval});" tabindex="-1">{$language.name|strval}</a></li>
													{/foreach}
												</ul>
											</div>
										</div>
										...
										<div class="col-lg-2">
											<button type="button" class="btn btn-default{if isset($input.button.attributes['class'])} {$input.button.attributes['class']}{/if}{if isset($input.button.class)} {$input.button.class}{/if}"
												{foreach from=$input.button.attributes key=name item=value}
													{if $name|lower != 'class'}
													 {$name|strval}="{$value|strval}"
													{/if}
												{/foreach} >
												{$input.button.label|strval}
											</button>
										</div>
									</div>
									{if isset($input.maxchar)}
										...
										<select name="{$input.name|escape:'html':'utf-8'}"
											...
											{if isset($input.options.optiongroup)}
												{foreach $input.options.optiongroup.query AS $optiongroup}
													<optgroup label="{$optiongroup[$input.options.optiongroup.label]|strval}">
														{foreach $optiongroup[$input.options.options.query] as $option}
															<option value="{$option[$input.options.options.id]|strval}"
																...
																{/if}
															>{$option[$input.options.options.name]|strval}</option>
														{/foreach}
													</optgroup>
												{/foreach}
											{else}
												{foreach $input.options.query AS $option}
													{if is_object($option)}
														<option value="{$option->$input.options.id|strval}"
															{if isset($input.multiple)}
																{foreach $fields_value[$input.name] as $field_value}
															...
															{else}
																...
																{/if}
															{/if}
														>{$option->$input.options.name|strval}</option>
													{elseif $option == "-"}
														<option value="">-</option>
													{else}
														<option value="{$option[$input.options.id]|strval}"
									...
									<div "col-lg-9">
										<span class="switch prestashop-switch fixed-width-lg">
											...
											<input
												...
												name="{$input.name|strval}"
												{if $value.value == 1}
													id="{$input.name|strval}_on"
												{else}
													id="{$input.name|strval}_off"
												{/if}
											...
											<label
												{if $value.value == 1}
													for="{$input.name|strval}_on"
												{else}
													for="{$input.name|strval}_off"
												{/if}
											>
												{if $value.value == 1}
													{l s='Yes' mod='prestacenterxmlexportfree'}
												{else}
													{l s='No' mod='prestacenterxmlexportfree'}
												{/if}
											</label>
											{/foreach}
									...
									{if $languages|count > 1}
										...
										<div class="col-lg-2">
											...
											<ul class="dropdown-menu">
												{foreach from=$languages item=language}
												<li>
													<a href="javascript:hideOtherLanguage({$language.id_lang|strval});" tabindex="-1">{$language.name|strval}</a>
												</li>
												{/foreach}
											</ul>
										</div>
								...
								{elseif $input.type == 'checkbox'}
                                    {if isset($input.expand)}
                                        <a class="btn btn-default show_checkbox{if $input.expand.default == 'hide'} hidden {/if}" href="#">
                                            <i class="icon-{$input.expand.show.icon|strval}"></i>
                                            {$input.expand.show.text|strval}
                                            {if isset($input.expand.print_total) && $input.expand.print_total > 0}
                                                <span class="badge">{$input.expand.print_total|strval}</span>
                                            {/if}
                                        </a>
                                        <a class="btn btn-default hide_checkbox{if $input.expand.default == 'show'} hidden {/if}" href="#">
                                            <i class="icon-{$input.expand.hide.icon|strval}"></i>
                                            {$input.expand.hide.text|strval}
                                            {if isset($input.expand.print_total) && $input.expand.print_total > 0}
                                                <span class="badge">{$input.expand.print_total|strval}</span>
                                            {/if}
                                        </a>
									...
									{foreach $input.values.query as $value}
										...
										<div class="checkbox{if isset($input.expand) && $input.expand.default == 'show'} hidden {/if}">
											<label for="{$id_checkbox|strval}">
												<input type="checkbox"
													name="{$id_checkbox|strval}"
													id="{$id_checkbox|strval}"
													class="{if isset($input.class)}{$input.class}{/if}"
													{if isset($value.val)}value="{$value.val|escape:'html':'UTF-8'}"{/if}
													{if isset($fields_value[$id_checkbox]) && $fields_value[$id_checkbox]}checked="checked"{/if} />
												{$value[$input.values.name]|strval}
											</label>
										</div>
									{/foreach}
								{elseif $input.type == 'change-password'}
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name|strval}-btn-change" class="btn btn-default">
												<i class="icon-lock"></i>
												{l s='Change password...' mod='prestacenterxmlexportfree'}
											</button>
											<div id="{$input.name|strval}-change-container" class="form-password-change well hide">
												<div class="form-group ">
													<label for="old_passwd" class="control-label col-lg-2 required">
														{l s='Current password' mod='prestacenterxmlexportfree'}
													</label>
													<div class="col-lg-10">
												...
												<hr>
												<div class="form-group">
													<label for="{$input.name|strval}" class="required control-label col-lg-2">
														<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="Minimum of 8 characters.">
															{l s='New password' mod='prestacenterxmlexportfree'}
														</span>
													</label>
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
															...
															<input type="password"
																id="{$input.name|strval}"
																name="{$input.name|strval}"
																class="{if isset($input.class)}{$input.class}{/if}"
																value=""
																required="required"/>
														</div>
														<span id="{$input.name|strval}-output"></span>
													</div>
												</div>
												<div class="form-group">
													<label for="{$input.name|strval}2" class="required control-label col-lg-2">
														{l s='Confirm password' mod='prestacenterxmlexportfree'}
													</label>
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															<span class="input-group-addon">
															...
															<input type="password"
																id="{$input.name|strval}2"
																name="{$input.name|strval}2"
																class="{if isset($input.class)}{$input.class}{/if}"
																value=""/>
														</div>
													</div>
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<input type="text" class="form-control fixed-width-md pull-left" id="{$input.name|strval}-generate-field" disabled="disabled">
														<button type="button" id="{$input.name|strval}-generate-btn" class="btn btn-default">
															<i class="icon-random"></i>
															{l s='Generate password' mod='prestacenterxmlexportfree'}
														</button>
													</div>
												</div>
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<p class="checkbox">
															<label for="{$input.name|strval}-checkbox-mail">
																<input name="passwd_send_email" id="{$input.name|strval}-checkbox-mail" type="checkbox" checked="checked">
																{l s='Send me this new password by Email' mod='prestacenterxmlexportfree'}
															</label>
														</p>
													</div>
												</div>
												<div class="row">
													<div class="col-lg-12">
														<button type="button" id="{$input.name|strval}-cancel-btn" class="btn btn-default">
															<i class="icon-remove"></i>
															{l s='Cancel' mod='prestacenterxmlexportfree'}
														</button>
													</div>
												</div>
											</div>
									...
									<script>
										$(function(){
											...
											var $passwordField = $('#{$input.name|strval}');
											var $output = $('#{$input.name|strval}-output');
											var $generateBtn = $('#{$input.name|strval}-generate-btn');
											var $generateField = $('#{$input.name|strval}-generate-field');
											var $cancelBtn = $('#{$input.name|strval}-cancel-btn');

											var feedback = [
												{ badge: 'text-danger', text: '{l s="Invalid" js=1 mod='prestacenterxmlexportfree'}' },
												{ badge: 'text-warning', text: '{l s="Okay" js=1 mod='prestacenterxmlexportfree'}' },
												{ badge: 'text-success', text: '{l s="Good" js=1 mod='prestacenterxmlexportfree'}' },
												{ badge: 'text-success', text: '{l s="Fabulous" js=1 mod='prestacenterxmlexportfree'}' }
											];
											...
											var $container = $('#{$input.name|strval}-change-container');
											var $changeBtn = $('#{$input.name|strval}-btn-change');
											var $confirmPwd = $('#{$input.name|strval}2');
											...
											}, '{l s="Invalid password confirmation" js=1 mod='prestacenterxmlexportfree'}');
										...
										<select name="{$key}" class="{if isset($input.class)}{$input.class}{/if}" class="fixed-width-lg">
											...
											{if $key == 'months'}
												{*
													...
													{l s='January' mod='prestacenterxmlexportfree'}
													{l s='February' mod='prestacenterxmlexportfree'}
													{l s='March' mod='prestacenterxmlexportfree'}
													{l s='April' mod='prestacenterxmlexportfree'}
													{l s='May' mod='prestacenterxmlexportfree'}
													{l s='June' mod='prestacenterxmlexportfree'}
													{l s='July' mod='prestacenterxmlexportfree'}
													{l s='August' mod='prestacenterxmlexportfree'}
													{l s='September' mod='prestacenterxmlexportfree'}
													{l s='October' mod='prestacenterxmlexportfree'}
													{l s='November' mod='prestacenterxmlexportfree'}
													{l s='December' mod='prestacenterxmlexportfree'}
												*}
												{foreach $select as $k => $v}
													<option value="{$k}" {if $k == $fields_value[$key]}selected="selected"{/if}>{l s=$v mod='prestacenterxmlexportfree'}</option>
												{/foreach}
											{else}
												{foreach $select as $v}
								...
								{elseif $input.type == 'categories'}
									{$categories_tree|strval}
								{elseif $input.type == 'file'}
									{$input.file|strval}
								{elseif $input.type == 'categories_select'}
									{$input.category_tree|strval}
								...
								{elseif $input.type == 'free'}
									{$fields_value[$input.name]|strval}
										...
										<p class="help-block">
											{if is_array($input.desc)}
												{foreach $input.desc as $p}
													{if is_array($p)}
														<span id="{$p.id|strval}">{$p.text|strval}</span><br />
													{else}
														{$p|strval}<br />
													{/if}
												{/foreach}
			...
			{block name="footer"}
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="panel-footer">
						...
						{/if}
						{* ----------- toto tlačítko je překryté ------------ *}
						{if isset($show_cancel_button) && $show_cancel_button}
						<a
							href="{if !empty($fieldset['form']['cancel']['href'])}{$fieldset['form']['cancel']['href']}{else}{$back_url}{/if}"
							class="btn btn-default{if !empty($fieldset['form']['cancel']['class'])} {$fieldset['form']['cancel']['class']}{/if}"
							{if empty($fieldset['form']['cancel']['remove_onclick'])}
							onclick="window.history.back()"
							{/if}
						>
							<i class="{if !empty($fieldset['form']['cancel']['icon'])}{$fieldset['form']['cancel']['icon']}{else}process-icon-cancel{/if}"></i>
							{if !empty($fieldset['form']['cancel']['title'])}{$fieldset['form']['cancel']['title']}
							{else}{l s='Cancel' mod='prestacenterxmlexportfree'}
							{/if}
						</a>
						{/if}
						{* ----- konec překryté části ------- *}
...
{if $firstCall}
	<script type="text/javascript">
		var module_dir = '{$smarty.const._MODULE_DIR_|strval}';
		var id_language = {$defaultFormLanguage|strval};
		...
		{foreach $languages as $k => $language}
			languages[{$k|strval}] = {
				id_lang: {$language.id_lang|strval},
				iso_code: '{$language.iso_code|strval}',
				name: '{$language.name|strval}',
				is_default: '{$language.is_default|strval}'
			};
		{/foreach}
		...
		$(document).ready(function() {
			...
			{if isset($fields_value.id_state)}
				...
				{
					ajaxStates({$fields_value.id_state|strval});
			...
			$('.datetimepicker').datetimepicker({
				...
				currentText: '{l s='Now' mod='prestacenterxmlexportfree'}',
				closeText: '{l s='Done' mod='prestacenterxmlexportfree'}',
				ampm: false,
				amNames: ['AM', 'A'],
				pmNames: ['PM', 'P'],
				timeFormat: 'hh:mm:ss tt',
				timeSuffix: '',
				timeOnlyTitle: '{l s='Choose Time' mod='prestacenterxmlexportfree'}',
				timeText: '{l s='Time' mod='prestacenterxmlexportfree'}',
				hourText: '{l s='Hour' mod='prestacenterxmlexportfree'}',
				minuteText: '{l s='Minute' mod='prestacenterxmlexportfree'}',
			});

...
<div class="banner-item clearfix" data-id="{$id_banner|intval}">
	...
	<div class="banner-details" style="display:none;">
		...
		<div class="form-group{if !isset($banner.content.$input_name)} empty{/if}">
			<label class="control-label col-lg-1">
				{if isset($field.tooltip)}
					<span class="label-tooltip" data-toggle="tooltip" title="{$field.tooltip|escape:'html'}">
				{/if}
					{$field.display_name|escape:'html'}
				{if isset($field.tooltip)}
					</span>
				{/if}
			...
			{foreach from=$languages item=lang}
				...
				{else if $input_name == 'link'}
					<select name="banner_data[{$id_lang|intval}][link][type]" class="col-lg-3 linkTypeSelector">
						{foreach $field.selector key=k item=type}
							<option value="{$k|escape:'html'}"{if $value && $value.type == $k} selected{/if}>{$type|escape:'html'}</option>
						{/foreach}
					</select>
					<div class="input-group link-type col-lg-9" data-type="{if $value && $value.type}{$value.type|escape:'html'}{else}custom{/if}">
						...
						<input type="text" name="banner_data[{$id_lang|intval}][link][href]" value="{if $value && $value.href}{$value.href|escape:'html'}{/if}">
						<span class="input-group-addon">
							<label class="label-checkbox">
				...
				{else if $input_name == 'html'}
					<textarea class="mce" name="banner_data[{$id_lang|intval}][{$input_name|escape:'html'}]" id="banner_{$id_banner|intval}_{$input_name|escape:'html'}_{$id_lang|intval}">{if $value}{$value}{/if}</textarea>
				{else if $input_name == 'class'}
					<div class="input-group all-langs">
						<input type="text" name="banner_data[{$id_lang|intval}][class]" value="{if $value}{$value|escape:'html'}{/if}">
				...
				{else}
					<input type="text" name="banner_data[{$id_lang|intval}][{$input_name|escape:'html'}]" id="banner_{$id_banner|intval}_{$input_name}|escape:'html'}_{$id_lang|intval}" value="{if $value}{$value|escape:'html'}{/if}" class="{if isset($field.all_langs)}all-langs{/if}">
				{/if}
				</div>
			{/foreach}
			</div>
			<div class="col-lg-1">
				...
				<div class="banner-langs">
					<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
						{foreach from=$languages item=lang}
							<span class="multilang lang-{$lang.id_lang|intval}" style="{if $lang.id_lang != $id_lang_current}display:none;{/if}">{$lang.iso_code|escape:'html'}</span>
						{/foreach}
		...
		{/foreach}
		<div class="p-footer">
			<input type="hidden" name="hook_name" value="{$banner.hook_name|escape:'html'}">

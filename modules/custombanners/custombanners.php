...
class CustomBanners extends Module
{
	public function __construct()
	{
		...
		$this->description = $this->l('Add custom banners');
	...
	public function uninstall()
	{
		...
		{
			$this->db->execute('DELETE FROM '._DB_PREFIX_.pSQL($table).' WHERE id_shop IN ('.implode(',', $shop_ids).')');
	...
	public function hookDisplayBackOfficeHeader()
	{
		...
		$this->context->controller->addCSS($this->local_path.'views/css/common-classes.css', 'all');
	...
	public function getContent()
	{
		...
		Media::addJsDef(array(
			'twa_ajax_path' => $this->context->language->id,
	...
	public function importBannersData($zip_file = false)
	{
		...
		foreach ($shop_ids as $id_shop)
		{
			...
			foreach (array('css', 'js') as $type)
			{
				$dest = $this->local_path.'views/'.$type.'/shop'.$id_shop.'.'.$type;
		...
		foreach ($tables_to_fill as $table_name => $rows_to_insert)
		{
			...
			$sql[] = 'DELETE FROM '._DB_PREFIX_.pSQL($table_name).' WHERE id_shop IN ('.implode(', ', $shop_ids).')';
			$rows = array();
			$column_names = array();
	...
	{
		if (!$shop_ids)
			$shop_ids = $shop_ids = Shop::getContextListShopID();
		$out_of_context = $this->db->executeS('
			SELECT content FROM '._DB_PREFIX_.'custombanners WHERE id_shop NOT IN ('.implode(',', $shop_ids).')
		');
		$imgs_to_keep = array();
	...
	public function getAllBannersData()
	{
		...
		$banners_db = $this->db->executeS('
			SELECT * FROM '._DB_PREFIX_.'custombanners WHERE id_shop IN ('.implode(',', $shop_ids).') ORDER BY position
		');
		$sorted = array();
	...
	public function callBannerForm($id_banner, $encode = true)
	{
		...
		$this->context->smarty->assign(array(
			...
			'languages' => Language::getLanguages(false),
			'id_lang_current' => $this->context->language->id,
			'multishop_note' => count($shop_ids) > 1,
		));
	...
	public function copyToAnotherHook($id_banner, $to_hook, $delete_original = false)
	{
		...
		if ($copied)
		{
			...
			if (!$this->isRegisteredInHook($to_hook))
				$this->registerHook($to_hook);
			if ($delete_original)
			{
		...
		switch ($action)
		{
			...
			case 'disable':
				...
				$success &= $this->db->execute('
					...
					WHERE id_banner IN ('.implode(', ', $banner_ids).') AND id_shop IN ('.implode(', ', $shop_ids).')
				');
			break;
			case 'move':
			case 'copy':
	...
	**/
	...
	public function ajaxToggleBannerParam()
	{
		...
		$update_query = '
			...
			WHERE id_banner = '.(int)$id_banner.' AND id_shop IN ('.implode(', ', $shop_ids).')
		';
	...
	public function deleteBanner($id_banner)
	{
		...
		foreach ($banner_shop_lang as $bsl)
		{
			...
			if (!isset($content['img']))
				continue;
			if (in_array($bsl['id_shop'], $shop_ids))

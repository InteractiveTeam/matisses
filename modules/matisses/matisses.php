...
class matisses extends Module
{
    ...
    public static function getCombinationMain($id_product,$ipa) {
    	$data = Db::getInstance()->getRow("SELECT default_on,itemname FROM "._DB_PREFIX_."product_attribute WHERE id_product = " . $id_product . "  AND id_product_attribute = " .$ipa);
    	return $data;
    }
	/***********************************************
	* BACKEND
	...
	*********************************************/
	...
	private function createFacebookLogin($CurrentCustomer)
	{
        ...
        {
            if(strpos(urldecode($back),_PS_BASE_URL_SSL_) === false)
...
/*		echo '<br><br><br>otra prueba<br><br><br>';
	...
	public function hookactionAddGarantia($params)
	{
		...
		$garantia['serviceRequestDTO']['itemCode'] 		= $params['itemCode']." 213123123123";
	...
	public function wsmatisses_registrar($params) 
	{
		...
		$order['incomingPaymentDTO']['franquicia'] = (!empty($params['franchise']) && $params['franchise'] == "_PSE_" ? $params['bank'] : $params['franchise_name']);
		$order['incomingPaymentDTO']['tipo']       = (!empty($params['franchise']) && $params['franchise'] == "_PSE_" ? 'DEBITO' : 'CREDITO');
